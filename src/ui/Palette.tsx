 ...
 *
 * This page is one box and a reading of what is in it. Fifteen colour chips above the fold
 * would be the loudest thing on a page whose whole argument is that it is quiet, so the list
 * stays closed until somebody wants it, and what shows until then is one line.
 */
            ...
            {g.themes.map((t) => (
              <button key={t.id} aria-pressed={t.id === theme} onClick={() => setTheme(t.id)}>
                {/* data-theme goes on the CHIP, not on the button. On the button it also
                    ...
                    the page is not showing, and most of them came out under 4.5:1 on the
